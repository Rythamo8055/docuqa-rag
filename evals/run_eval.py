...
def main() -> int:
    ...
    report = render_report(out, args.llm)
...
def render_report(out: dict, llm_mode: bool) -> str:
    ...
    lines = [
        ...
        f"- Avg faithfulness (answer↔context overlap): **{s['avg_faithfulness']}**",
        f"- Avg relevance (question→answer coverage): **{s['avg_relevance']}**",
        "",
        "## Guardrail checks",
        "",
    ...
    lines += ["", "## Per-case results", "", "| ID | Category | Pass | Blocked | Grounded | Pages | Term cov | Faith | Rel |", "|---|---|---|---|---|---|---|---|---|"]
    for r in results:
        lines.append(
